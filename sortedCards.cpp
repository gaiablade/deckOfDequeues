/* sortedCards.cpp
 * Similar to the normal rules. However, after each round, the players cards are sorted.
 * This can cause results to be different if the game ends by no more turns.
 */
#include "Cards.h"
#include <iostream>
#include <sstream>
...
int main(int argc, char** argv) {
    ...
    while (currentTurn < numTurns + 1 && !winner) { // while there is no winner and turn limit isn't reached
        ...
        // SPECIAL GAMERULE: Keep card piles sorted.
        players[givingPlayer].cardPile.BubbleSort();
        players[acceptingPlayer].cardPile.BubbleSort();
